...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT * FROM media WHERE id = $propertyId";
...
if (isset($_POST['mediaSubmit'])) {
    ...
    // File upload handling (if you want to allow updating the image)
    if ($_FILES['mediaImage']['size'] > 0) {
        // Process the file upload code here
        $targetDir = "mediaImage/";
        $targetFile = $targetDir . basename($_FILES["mediaImage"]["name"]);

        // Add your file upload validation and move_uploaded_file logic here
        $uploadOk = 1;
        ...
        if ($uploadOk) {
            if (move_uploaded_file($_FILES["mediaImage"]["tmp_name"], $targetFile)) {
                echo '<script>alert("The file ' . htmlspecialchars(basename($_FILES["mediaImage"]["name"])) . ' has been uploaded.");</script>';
            } else {
        ...
        // If no new image is uploaded, you can keep the existing image path
        $targetFile = $row_data['mediaImage'];
    }

    // Validate other form fields if needed
    if (empty($mediaTitle) || empty($mediaShortDesc) || empty($mediaLongDesc)) {
        echo '<script>alert("Please fill in all the required fields.");</script>';
    } else {
        // Update data in the database
        $updateSql = "UPDATE media SET mediaTitle = '$mediaTitle', mediaShortDesc = '$mediaShortDesc', mediaLongDesc = '$mediaLongDesc', mediaImage = '$targetFile' WHERE id = $propertyId";

        if ($conn->query($updateSql) === TRUE) {
            echo '<script>alert("Record updated successfully");</script>';
            echo '<script type="text/javascript">window.location.href="../listMedia.php";</script>';
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form class="form-sample" method="post" enctype="multipart/form-data">
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <input type="text" name="mediaTitle" value="<?php echo $row_data['mediaTitle'] ?>" class="form-control" />
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <input type="text" name="mediaShortDesc" value="<?php echo $row_data['mediaShortDesc'] ?>" class="form-control" />
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-9">
                                                        <input type="text" name="mediaLongDesc" value="<?php echo $row_data['mediaLongDesc'] ?>" class="form-control" />
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                        ...
                                        <button type="submit" name="mediaSubmit" class="btn btn-primary mr-2">Update</button>
