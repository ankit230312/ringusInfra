...
if (isset($_POST['submit'])) {
    ...
    $propertyPincode = isset($_POST['propertyPincode']) ? mysqli_real_escape_string($conn, $_POST['propertyPincode']) : '';

    // Check if the pincode is not empty
    if (!empty($propertyPincode)) {
        // Check if the pincode already exists
        $checkQuery = "SELECT * FROM pincode WHERE pincode = '$propertyPincode'";
        $checkResult = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($checkResult) > 0) {
            // Pincode already exists, show an error message
            echo "<script>alert('Duplicate Pincode!')</script>";
            echo "Duplicate Pincode!";
        } else {
            // Pincode does not exist, proceed to insert
            // Construct the SQL query
            $query = "INSERT INTO pincode (pincode, created, updated) VALUES ('$propertyPincode', CURDATE(), CURDATE())";
            ...
            if ($result) {
                ...
                echo "Error adding pincode: " . mysqli_error($conn);
            }
        }
    } else {
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group row">
                                        <label class="col-sm-3 col-form-label">Add Pincode</label>
                                        <div class="col-sm-9">
                                            <input type="number" name="propertyPincode" class="form-control" required />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <tbody>
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo "<a href='functionFiles/edit_pincode.php?id={$row['id']}' title='Edit'><i class='mdi mdi-pencil'></i></a>";
                                            echo "&nbsp;&nbsp;";
                                            echo "<a href='functionFiles/delete_pincode.php?id={$row['id']}' title='Delete' onclick='return confirm(\"Are you sure?\")'><i class='mdi mdi-delete'></i></a>";
                                            echo "</td>";
                                            echo "</tr>";
                                        }
                                    } else {
