    ...
    return this.each(function() {
      ...
      function checkResults(term) {
        if ($list.children().size() === 0) {
          $noResults.php(opts.noResults(term)).show()
        } else {
          $noResults.hide()
          listNavigator.ensure()
        }
      ...
      function initialSelection(useOptsInitial) {
        ...
        if (useOptsInitial && opts.initial) {
          ...
          $selected.php(opts.placeholder).addClass('placeholder')
        } else {
          var first = $list.find(':first').data('zelect-item')
          first !== undefined ? selectItem(first) : $selected.php(opts.noResults()).addClass('placeholder')
        }
        checkResults()
      }
    })
