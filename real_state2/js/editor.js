	...
	var methods = {
		...
		init : function( options )
		{
			...
			var menuItems = { 'fonteffects': true,
							 ...
							 'font_size': {	"select":true,
							  ...
							  'color'	: { "text":"A",
											...
											"custom":function(button){
													...
													var palette       = $('<ul />',{id:"color_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).php('<div>Text Color</div>'));
													var bgPalletteDiv = $('<div/>',{id:"bg_colorpellete"});
													var bgPallette    = $('<ul />',{id:"bgcolor_ui"}).append($('<li />').css({"width":"145px","display":"Block","height":"25px"}).php('<div>Background Color</div>'));
						...
						'insert_table'	: { "modal": true,
											...
											"onSave": function(){
												...
												if(navigator.userAgent.match(/MSIE/i))
												methods.restoreSelection.apply(this,[htmlTableCntr.php(),'html']);
												else
												document.execCommand('insertHTML', false, htmlTableCntr.php());
						   ...
						   'outdent'	: { "text": "Outdent", 
							...
							'print'		: { "text": "Print", 
											...
											oPrntWin.document.write("<!doctype html><html><head><title>Print</title></head><body onload=\"print();\">" + oDoc.php() + "</body></html>");
											...
											"custom":function(button){
													...
													if(flag==0){
														for (var i = 0; i < specialchars.length; i++){															
															splCharUi.append($('<li />').php(specialchars[i].text).attr('title',specialchars[i].name).mousedown(function(event){ event.preventDefault();}).click(function(event){
																if(navigator.userAgent.match(/MSIE/i)){
																	var specCharHtml = $(this).php();
																	methods.insertTextAtSelection.apply(this,[specCharHtml,'html']);
																}
																else{
																	document.execCommand('insertHTML',false,$(this).php());
																}
																$('#specialchar').remove();
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").php('<div class="label">'+'Words : '+wordCount+'</div>');
		    ...
		    $(document.body).mousedown(function(event) {
		        ...
		        if (!target.parents().andSelf().is('#specialchar') && (target.closest('a').php()!='<i class="fa fa-asterisk"></i>')) { //Clicked outside
		        ...
		        if (!target.parents().andSelf().is('#paletteCntr') && (target.closest('a').php()!='<i class="fa fa-font"></i>')) { //Clicked outside
											...
											"text":"Add Row"
												...
												return function(){
													...
													selectedRow.children().each(function() {
														var newColumn = $("<" + $(this).prop("nodeName") + "/>").php("&nbsp;");
										...
										"text":"Add Column",
											...
											return function(){
												...
												selectedCell.closest("table").find("tr").each(function() {
													...
													var newCell = $("<" + cellInSelectedColumn.prop("nodeName") + "/>").php("&nbsp;");
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				menuWrapElement.append($('<a/>',{
										...
										}).php(itemSettings["default"]).append($("<span/>",{class:"caret"})).mousedown(function(e){
											e.preventDefault();
										}));
		    	...
		    	if(itemSettings["icon"])
					...
					menuElement.php(itemSettings["text"]);
		...
		getSource: function(button, params){
			...
			if(flag==0){ //Convert text to HTML			
				content = document.createTextNode(editor.php());
				editor.empty();
